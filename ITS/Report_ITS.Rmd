...
#Chao Plot
alpha_fungi_senegal <- alpha_diversity_plot(physeq.senegal, "Microhabitat", metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Senegal") + theme(strip.text = element_blank(), axis.title.y=element_blank(),  legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
alpha_fungi_senegal
...
#Chao Plot
alpha_fungi_spain <- alpha_diversity_plot(physeq.spain, "Microhabitat", metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Spain") + theme(strip.text = element_blank(), axis.title.y=element_blank(), legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) 
alpha_fungi_spain
...
#Chao Plot
alpha_fungi_under <- alpha_diversity_plot(physeq.understory, "Country", metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Under") + theme(strip.text = element_blank(), axis.title.y=element_blank(), legend.position = "none", axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + scale_fill_manual(values = c("Spain" = "#E69F00", "Senegal" = "#C3D7A4"))
alpha_fungi_under
...
beta_fungi <- plot_distance(physeq.10, pcoa_bray, "Microhabitat", shape="Country", elipse = TRUE) + annotate("text" ,x=0.2,y=0.3, label = bray_permanova_text, size = 5) 
beta_fungi
```
...
```{r TopGenus, fig.width=20, fig.height=8}
...
top_fungi <- ggplot(top_sumarize, aes(x = Relative_Abundance, y = Genus, fill = Microhabitat)) +
...
senegal.daa.fungi.barplot <- daa_bar_plot(sigtab.senegal, contrast, "Senegal      \nGap // Understory", col_pal = FALSE) + 
  theme(
...
spain.daa.fungi.barplot <- daa_bar_plot(sigtab.spain, contrast, "Spain       \nGap // Understory", col_pal = FALSE) + 
  theme(
  plot.title = element_text(hjust = 0.9, size = 18), 
  axis.text=element_text(size=12), 
  axis.title=element_text(size=14,face="bold"),
...
ggarrange(senegal.daa.fungi.barplot, spain.daa.fungi.barplot)
```
...
ven_fungi <- ggVennDiagram(lst3, label_alpha = 0, set_size = 6, label_size = 6) +  scale_fill_gradient(low = "#F4FAFE", high = "#4981BF") +  theme(legend.position = "none") + scale_x_continuous(expand = expansion(mult = .2))
ven_fungi 
```
