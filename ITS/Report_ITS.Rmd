...
#Chao Plot
alpha_bact_under <- alpha_diversity_plot(physeq.understory, "Country", paper_metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Under") + theme(strip.text = element_blank(), axis.title.y=element_blank())

#Obtain alpha-diversity plot complete
alpha_diversity_plot(physeq.understory, "Country", metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Understory")
...
#Chao Plot
alpha_bact_gap <- alpha_diversity_plot(physeq.gap, "Country", paper_metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Gap") + theme(strip.text = element_blank())
...
ps.spain.understory <- subset_samples(ps.genus.rel, Microhabitat == "Understory" & Country == "Spain")
top.spain.understory <- top_taxa(ps.spain.understory, n = 20)
ps.top.spain.understory <- prune_taxa(top.spain.understory, ps.spain.understory)

#Obtain Spain Gap top 20 genera
ps.spain.gap <- subset_samples(ps.genus.rel, Microhabitat == "Gap" & Country == "Spain")
top.spain.gap <- top_taxa(ps.spain.gap, n = 20)
ps.top.spain.gap <- prune_taxa(top.spain.gap, ps.spain.gap)

#Obtain Senegal Understory top 20 genera
ps.senegal.understory <- subset_samples(ps.genus.rel, Microhabitat == "Understory" & Country == "Senegal")
top.senegal.understory <- top_taxa(ps.senegal.understory, n = 20)
ps.top.senegal.understory <- prune_taxa(top.senegal.understory, ps.senegal.understory)

#Obtain Senegal Gap top 20 genera
ps.senegal.gap <- subset_samples(ps.genus.rel, Microhabitat == "Gap" & Country == "Senegal")
top.senegal.gap <- top_taxa(ps.senegal.gap, n = 20)
ps.top.senegal.gap <- prune_taxa(top.senegal.gap, ps.senegal.gap)

ps.top.genus <- merge_phyloseq(ps.top.spain.understory, 
                               ps.top.spain.gap, 
                               ps.top.senegal.understory, 
                               ps.top.senegal.gap)
...
### Differences in microhabitats per each Country
...
spain.daa.barplot <- daa_bar_plot(sigtab.spain, contrast, "Spain       \nGap // Understory") + 
  theme(
  plot.title = element_text(hjust = 0.9, size = 18), 
  axis.text=element_text(size=12), 
  axis.title=element_text(size=14,face="bold"),
...
```{r DAABarplotCountry, fig.width=16, fig.height=14}
ggarrange(senegal.daa.barplot, spain.daa.barplot)
```
  
### Differences in the countries per each microhabitat
  
```{r DAAUnderstory}
physeq.understory.genus <- tax_glom(physeq.understory, taxrank="Genus")
understory_genus_l <- get_taxa_unique(physeq.understory.genus, taxonomic.rank = "Genus")
understory_genus_l <- understory_genus_l[!understory_genus_l %in% genus_to_remove]
physeq.understory.genus <- subset_taxa(physeq.understory.genus, Genus %in% understory_genus_l)
alpha <- 0.05
ds_understory <- phyloseq_to_deseq2(physeq.understory.genus, ~ Country)


diagdds_understory <-  DESeq(ds_understory, 
                  test="Wald", 
                  fitType="parametric", 
                  sfType = 'poscounts')
res.understory <- results(diagdds_understory, cooksCutoff = F,
               contrast = c("Country", "Senegal", "Spain") )
sigtab.understory <- res.understory[which(res.understory$padj < alpha), ]
sigtab.understory <- cbind(as(sigtab.understory, "data.frame"), as(tax_table(physeq.understory.genus)[rownames(sigtab.understory), ], "matrix"))

understory.daa.barplot <- daa_bar_plot(sigtab.understory, contrast, "Understory   \nSpain // Senegal") + 
  theme(
...
```{r DAAGap}
physeq.gap.genus <- tax_glom(physeq.gap, taxrank="Genus")
gap_genus_l <- get_taxa_unique(physeq.gap.genus, taxonomic.rank = "Genus")
gap_genus_l <- gap_genus_l[!gap_genus_l %in% genus_to_remove]
physeq.gap.genus <- subset_taxa(physeq.gap.genus, Genus %in% gap_genus_l)
alpha <- 0.05
ds_gap <- phyloseq_to_deseq2(physeq.gap.genus, ~ Country)


diagdds_gap <-  DESeq(ds_gap, 
                  test="Wald", 
                  fitType="parametric", 
                  sfType = 'poscounts')
res.gap <- results(diagdds_gap, cooksCutoff = F,
               contrast = c("Country", "Senegal", "Spain") )
sigtab.gap <- res.gap[which(res.gap$padj < alpha), ]
sigtab.gap <- cbind(as(sigtab.gap, "data.frame"), as(tax_table(physeq.gap.genus)[rownames(sigtab.gap), ], "matrix"))

gap.daa.barplot <- daa_bar_plot(sigtab.gap, contrast, "Gap        \nSpain // Senegal") + 
  theme(
  plot.title = element_text(hjust = 0.9, size = 20), 
  axis.text=element_text(size=14), 
  axis.title=element_text(size=16,face="bold"),
...
```{r DAABarplotMicrohabitat, fig.width=18, fig.height=26}
ggarrange(understory.daa.barplot, gap.daa.barplot)
```
...
plt_senegal <- ggplot(df.senegal.mean, aes(x = Understory, y = Gap)) +
  ...
  scale_x_log10() + scale_y_log10() +  #Logarithmic scale as there are small values
...
#Plot Venn Diagram
lst1 <- list(Gap= ps.senegal.gap.l,
            Unde= get_taxa_unique(ps.senegal.understory.genus1, taxonomic.rank="Genus"))
...
#Plot Venn Diagram
lst2 <- list(Gap= ps.spain.gap.l,
            Unde= get_taxa_unique(ps.spain.understory.genus1, taxonomic.rank="Genus"))
