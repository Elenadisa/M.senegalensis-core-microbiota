...
The table shows ASVs in the negative control.
...
#Count the number of unique families per sample
family_count_sample_df <- family_df %>%
  group_by(SampleID) %>%
  summarise(Family_nb = length(unique(Family)))
family_count_sample_df$Country <- sam_data(physeq.clean)$Country
family_count_sample_df$Microhabitat <- sam_data(physeq.clean)$Microhabitat
...
As we can see in the table there are not significant differences in terms of the number of different families in the microhabitats of the same country. In this case, there are not significant differences between Senegal microhabitas, but there is a significant differences between Spain understory and gap.
...
As we can see in the table there are not significant differences in terms of number of phyla in the different microhabitats.  
...
#Chao Plot
alpha_bact_senegal <- alpha_diversity_plot(physeq.senegal, "Microhabitat", paper_metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Senegal") + theme(strip.text = element_blank(), axis.title.y=element_blank())

#Obtain alpha-diversity plot complete
alpha_diversity_plot(physeq.senegal, "Microhabitat", metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Senegal")
...
#Chao Plot
alpha_bact_spain <- alpha_diversity_plot(physeq.spain, "Microhabitat", paper_metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Spain") + theme(strip.text = element_blank(), axis.title.y=element_blank())

#Obtain alpha-diversity plot complete
alpha_diversity_plot(physeq.spain, "Microhabitat", metrics, my_comparisons_microhabitat, stat_metrics, asterisk = FALSE, "Spain")
...
The alpha diversity between both experimental conditions is not significantly different for Chao index, but it is significant for Shannon and Simpson indices, with the Kruskal wallys test and a p-value adjustment with BH. This indicates that within each group or condition, the communities may have different diversity in terms of the number of species or relative abundance. 
...
#Chao Plot
alpha_bact_under <- alpha_diversity_plot(physeq.in, "Country", paper_metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Under") + theme(strip.text = element_blank(), axis.title.y=element_blank())

#Obtain alpha-diversity plot complete
alpha_diversity_plot(physeq.in, "Country", metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Understory")

#alpha diversity table
in.tb <- estimate_richness(physeq.in, measures=metrics)
in.tb$Country <- sample_data(physeq.in)$Country
in.tb$Microhabitat <- rep("Understory", nrow(sample_data(physeq.in)))
kable(in.tb)
```
...
#Chao Plot
alpha_bact_gap <- alpha_diversity_plot(physeq.out, "Country", paper_metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Gap") + theme(strip.text = element_blank())

#Obtain alpha-diversity plot complete
alpha_diversity_plot(physeq.out, "Country", metrics, my_comparisons_country, stat_metrics, asterisk = FALSE, "Gap")

#alpha diversity table
out.tb <- estimate_richness(physeq.out, measures=metrics)
out.tb$Country <- sample_data(physeq.out)$Country
out.tb$Microhabitat <- rep("Gap", nrow(sample_data(physeq.out)))
kable(out.tb)
```
...
The main phyla are available in all the microhabitats.  
...
#Obtain Spain Understory top 20 genera 
ps.spain.in <- subset_samples(ps.genus.rel, Microhabitat == "Understory" & Country == "Spain")
top.spain.in <- top_taxa(ps.spain.in, n = 20)
ps.top.spain.in <- prune_taxa(top.spain.in, ps.spain.in)

#Obtain Spain Gap top 20 genera
ps.spain.out <- subset_samples(ps.genus.rel, Microhabitat == "Gap" & Country == "Spain")
top.spain.out <- top_taxa(ps.spain.out, n = 20)
ps.top.spain.out <- prune_taxa(top.spain.out, ps.spain.out)

#Obtain Senegal Understory top 20 genera
ps.senegal.in <- subset_samples(ps.genus.rel, Microhabitat == "Understory" & Country == "Senegal")
top.senegal.in <- top_taxa(ps.senegal.in, n = 20)
ps.top.senegal.in <- prune_taxa(top.senegal.in, ps.senegal.in)

#Obtain Senegal Gap top 20 genera
ps.senegal.out <- subset_samples(ps.genus.rel, Microhabitat == "Gap" & Country == "Senegal")
...
sigtab.senegal <- cbind(as(sigtab.senegal, "data.frame"), as(tax_table(physeq.senegal)[rownames(sigtab.senegal), ], "matrix"))

senegal.daa.barplot <- daa_bar_plot(sigtab.senegal, contrast, "Senegal      \nGap // Understory") + theme(plot.title = element_text(hjust = 0.9))
```
...
**4:** filter low abundant taxa (less than 0.15%).  
...
ps.senegal.out.genus_rel = transform_sample_counts(ps.senegal.out.genus, function(x){(x / sum(x))*100})
#obtain mean abundance per genus
df.senegal.out <- psmelt(ps.senegal.out.genus_rel)
df.senegal.out_mean <- df.senegal.out %>% group_by(Genus) %>% summarize(Gap=mean(Abundance, na.rm=TRUE))
...
#Calculate relative abundance 
ps.senegal.in.genus_rel = transform_sample_counts(ps.senegal.in.genus, function(x){(x / sum(x))*100})
#obtain mean abundance per genus
df.senegal.in <- psmelt(ps.senegal.in.genus_rel)
df.senegal.in_mean <- df.senegal.in %>% group_by(Genus) %>% summarize(Understory=mean(Abundance, na.rm=TRUE))

df.senegal.mean <- merge(df.senegal.out_mean, df.senegal.in_mean, by = "Genus") %>%
 replace(is.na(.), 0)

plt_senegal <- ggplot(df.senegal.mean, aes(x = Understory, y = Gap)) +
  geom_point(alpha = 0.7) +
  scale_x_log10() + scale_y_log10() +  # Escala logarítmica si hay valores muy pequeños
  geom_abline(slope = 1, intercept = 0, color = "red") + 
  geom_vline(xintercept = 0.007, linetype = "dashed", color = "darkgreen", size = 1) +
  geom_hline(yintercept = 0.007, linetype = "dotted", color = "blue", size = 1) +
...
ps.spain.out <- subset_samples(physeq.css, Microhabitat == "Gap" & Country == "Spain")
...
#Calculate relative abundance
ps.spain.out.genus_rel = transform_sample_counts(ps.spain.out.genus, function(x){(x / sum(x))*100})
#obtain mean abundance per genus
df.spain.out <- psmelt(ps.spain.out.genus_rel)
df.spain.out_mean <- df.spain.out %>% group_by(Genus) %>% summarize(Gap=mean(Abundance, na.rm=TRUE))

#Obtain A subset from spain Understory Samples
ps.spain.in <- subset_samples(physeq.css, Microhabitat == "Understory" & Country == "Spain")
...
ps.spain.in.genus_rel = transform_sample_counts(ps.spain.in.genus, function(x){(x / sum(x))*100})
#obtain mean abundance per genus
df.spain.in <- psmelt(ps.spain.in.genus_rel)
df.spain.in_mean <- df.spain.in %>% group_by(Genus) %>% summarize(Understory=mean(Abundance, na.rm=TRUE))

df.spain.mean <- merge(df.spain.out_mean, df.spain.in_mean, by = "Genus") %>%
 replace(is.na(.), 0)

plt_spain <- ggplot(df.spain.mean, aes(x = Understory, y = Gap)) +
  geom_point(alpha = 0.7) +
  scale_x_log10() + scale_y_log10() +  # Logaritmic scale
  geom_abline(slope = 1, intercept = 0, color = "red") + 
  geom_vline(xintercept = 0.007, linetype = "dashed", color = "darkgreen", size = 1) +
  geom_hline(yintercept = 0.007, linetype = "dotted", color = "blue", size = 1) +
  labs(x = "Relative Abundance Understory", 
...
ps.senegal.out.genus1 <- filter_taxa(ps.senegal.out.genus_rel, function(x){ mean(x) >= 0.2}, prune = TRUE)
#Obtain Senegal gap genus list
ps.senegal.out.l <- get_taxa_unique(ps.senegal.out.genus1, taxonomic.rank="Genus")

#Filter Senegal Understory by relative abundance
ps.senegal.in.genus1 <- filter_taxa(ps.senegal.in.genus_rel, function(x){ mean(x) >= 0.2}, prune = TRUE)

#Plot Venn Diagram
lst1 <- list(Gap= ps.senegal.out.l,
            Unde= get_taxa_unique(ps.senegal.in.genus1, taxonomic.rank="Genus"))

ven_sen_bact <- ggVennDiagram(lst1, label_alpha = 0, set_size = 6, label_size = 6) +  scale_fill_gradient(low = "#F4FAFE", high = "#4981BF") +  theme(legend.position = "none") + scale_x_continuous(expand = expansion(mult = .2)) + ggtitle("Senegal")
ven_sen_bact 
```

```{r SenegalExclusive}
#Remove Gap genera in Understory
senegal_in_exclusive <- subset_taxa(ps.senegal.in.genus1, !Genus %in% ps.senegal.out.l)
senegal_in_exclusive.l <- get_taxa_unique(senegal_in_exclusive, taxonomic.rank="Genus")

#Obtain Genera that are in the 100% samples
senegal_in_exclusive.all <- prune_taxa(rowSums(otu_table(senegal_in_exclusive) != 0) == ncol(otu_table(senegal_in_exclusive)), senegal_in_exclusive)

pander(paste("There are ", length(senegal_in_exclusive.l)," exclusive genera in Senegal understory: ",paste(senegal_in_exclusive.l, collapse = ", ")))  
pander(paste("Among all the exclusive genera in Senegal understory,", length(get_taxa_unique(senegal_in_exclusive.all, taxonomic.rank="Genus"))," are within all the samples: ",paste(get_taxa_unique(senegal_in_exclusive.all, taxonomic.rank="Genus"), collapse = ", ")))  
```
...
ps.spain.out.genus1 <- filter_taxa(ps.spain.out.genus_rel, function(x){ mean(x) >= 0.2}, prune = TRUE)
#Obtain list of Spain Gap genera
ps.spain.out.l <- get_taxa_unique(ps.spain.out.genus1, taxonomic.rank="Genus")

#Filter Spain Understory by relative Abundance
ps.spain.in.genus1 <- filter_taxa(ps.spain.in.genus_rel, function(x){ mean(x) >= 0.2}, prune = TRUE)

#Plot Venn Diagram
lst2 <- list(Gap= ps.spain.out.l,
            Unde= get_taxa_unique(ps.spain.in.genus1, taxonomic.rank="Genus"))

ven_sp_bact <- ggVennDiagram(lst2, label_alpha = 0, set_size = 6, label_size = 6) +  scale_fill_gradient(low = "#F4FAFE", high = "#4981BF") +  theme(legend.position = "none") + scale_x_continuous(expand = expansion(mult = .2)) + ggtitle("Spain")
ven_sp_bact 
```

```{r SpainExclusive}
#Remove Gap genera
spain_in_exclusive <- subset_taxa(ps.spain.in.genus1, !Genus %in% ps.spain.out.l)
spain_in_exclusive.l <- get_taxa_unique(spain_in_exclusive, taxonomic.rank="Genus")
pander(paste("There are ", length(spain_in_exclusive.l)," exclusive genera in Spain understory: ",paste(spain_in_exclusive.l, collapse = ", ")))

#Obtain Genera that are in the 100% samples
spain_in_exclusive.all <- prune_taxa(rowSums(otu_table(spain_in_exclusive) != 0) == ncol(otu_table(spain_in_exclusive)), spain_in_exclusive)
pander(paste("Among all the exclusive genera in Spain understory,", length(get_taxa_unique(spain_in_exclusive.all, taxonomic.rank="Genus"))," are within all the samples: ",paste(get_taxa_unique(spain_in_exclusive.all, taxonomic.rank="Genus"), collapse = ", ")))  
```
...
```{r AllVenn}
lst3 <- list(Sp.Gap= ps.spain.out.l,
            Sp.Unde= get_taxa_unique(ps.spain.in.genus1, taxonomic.rank="Genus"),
            Sn.Gap= ps.senegal.out.l,
            Sn.Unde= get_taxa_unique(ps.senegal.in.genus1, taxonomic.rank="Genus"))
...
```{r CoreMicrobiota}
core_microbiota <- intersect(get_taxa_unique(spain_in_exclusive, taxonomic.rank = "Genus"), get_taxa_unique(senegal_in_exclusive, taxonomic.rank = "Genus"))
pander(paste("Common genera between Senegal and Spain exclusive understory are: ", paste(core_microbiota, collapse=", ")))


core_microbiota_all <- intersect(get_taxa_unique(spain_in_exclusive.all, taxonomic.rank = "Genus"), get_taxa_unique(senegal_in_exclusive.all, taxonomic.rank = "Genus"))
